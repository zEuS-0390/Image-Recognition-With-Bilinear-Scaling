...
class Detector:
    ...
    # Load the model
    def loadModel(self):
        self.network = cv2.dnn.readNet(os.path.join(self.root, REQ_DIR_NAME, self.conf["Requirements"]["weights"]), 
                                       os.path.join(self.root, REQ_DIR_NAME, self.conf["Requirements"]["cfg"]))
        ...
        with open(os.path.join(self.root, REQ_DIR_NAME, self.conf["Requirements"]["names"]), "r") as file:
    ...
    def detect(self, image, NMS=False):
        ...
        blob = cv2.dnn.blobFromImage(image, 1/255, 
                                    (self.conf.getint("Detection", "nn_width"), 
                                     self.conf.getint("Detection", "nn_height")), 
        ...
        for out in outputs:
            for detected in out:
                ...
                if confidence > self.conf.getfloat("Detection", "confidence_thresh"):
...
class Detection(object):
    ...
    # Constructor
    def __init__(self, root, conf):
        self.OBJDetector = Detector(root, conf)
